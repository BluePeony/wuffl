...
output_dimensions = `system_profiler SPDisplaysDataType |grep Resolution`.chomp
...
IMG_MAX_W = (WINDOW_WIDTH * 0.43).to_i #max width for each image
IMG_MAX_H = (WINDOW_HEIGHT * 0.43).to_i #max height for each image
...
next_btn.signal_connect("clicked") do 
	...
	ind = set_next_index(ind, all_orig_pb)
	filename = dir_path + "/" + all_orig_img[ind]
	is_landscape = img_dimensions_fi(filename)[0]
	pb_current = all_orig_pb[ind]
	show_img(img_current, pb_current)
